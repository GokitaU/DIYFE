...
$(document).ready(function () {
    ...
    $.fn.RunNBATeamScrap = function (team) {
        ...
        var html = $('<div></div>');
        var html2 = $('<div></div>');
        var urlOne = team.ScrapUrl.replace('[half]', 1);
        var urlTwo = team.ScrapUrl.replace('[half]', 2);

        html.load(urlOne + ' #my-teams-table', function (response, status, xhr) {
            if (status == "error") {
                ...
                html2.load(urlTwo + ' #my-teams-table', function (response, status, xhr) {
                    if (status == "error") {
                        mediator.publish('pageError', { error: xhr.status + " " + xhr.statusText, method: 'loading ' + team.ScrapUrl + ' for scrapping - -' });
                    } else {

                        htmlHolder.append(html2);
                    }
                });
    ...
    $('#runScrapingLnk').click(function (event) {
        ...
        if (selectedTeam == '00') {
            ...
            $.ajax({
                url: GetRootURL() + 'Sports/GetAllTeams?leagueId=1',
                type: 'POST',
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.success) {
                        ...
                        //data.team.Scrap2Url = data.team.ScrapUrl.replace('[half]', selectedSeason);
                        //data.team.Scrap2Url = data.team.ScrapUrl.replace('[half]', selectedSeason);
    ...
    $('#saveScrappingLnk').click(function (event) {
        ...
        htmlHolder.find('.oddrow, .evenrow').each(function (index) {
            ...
            model.GameDate = $(this).find('td:eq(0) nobr').html() + ' ' + year;
        ...
        alert(games.length);
        $.ajax({
            url: GetRootURL() + 'sports-betting/mlb/InsertMLBGameScrap',
